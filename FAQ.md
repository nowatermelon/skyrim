...
Bread has most of the frameworks ready, including [OStim](https://www.nexusmods.com/skyrimspecialedition/mods/98163) for NSFW content. Check the mod description when adding animations to see if you need to rerun [Pandora](https://www.nexusmods.com/skyrimspecialedition/mods/133232). If you're unsure about the load order, use [LOOT](https://www.nexusmods.com/site/mods/439) to sort any new ESPs you install.
...
It might be caused by Upscaling (I initially selected DLSS + Frame Generation). You can try changing the Upscaling section in `overwrite\SKSE\Plugins\CommunityShaders\SettingsUser.json` to match the values in `SettingsDefault.json`. Then launch the game and set it to whatever you want.
...
If for any reason you choose to start a new game rather than using my provided save, use MCM Recorder to apply my custom MCM settings (for Bathing in Skyrim, GT Softbody, moreHUD, OBody NG, Simple Hunting Overhaul, Smoothcam, Spell Hotbar, Widget Addon.)
